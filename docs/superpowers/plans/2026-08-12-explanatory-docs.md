...
**§ The synthetic-lethal window.** The three conditions and why each exists: pS′_WT > 0 makes the effect an interpretable inhibition rather than a disinhibition or a null; pS′_mutant > 0 makes the dependency pharmacologically accessible rather than merely nominal; ΔpS′ ≤ −2 imposes a minimum effect size. Add the repo-level fourth condition, `MIN_LINES = 3`, and state what it actually counts: measurement coverage, not pool size — for each compound separately, at least three *non-missing* S′ values in the wild-type pool and three in the mutant pool (`blocking_analyses.py:22`). Never describe it as a cohort-size floor. Give the observable consequence: PTEN's mutant pool holds exactly three lines, so a compound missing a measurement in any one of them drops out, leaving 883 testable compounds for PTEN against 1,402 for CDKN2A and TP53. Then give the fold-change reading: because the window forces candidates into the pS′ ≈ 2–4 band, ΔpS′ ≤ −2 amounts in practice to requiring roughly a 7.4-fold (e²) greater potency–efficacy ratio in mutant cells, and `sprime_core.fold_change` computes exactly this. Note the correspondence degrades below pS′ ≈ 1. Follow with diagram D4.
